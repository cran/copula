...
c	if ((iparam .le. 0) .or. (iparam .gt. 3)) then
	  gamma0 = gammai
	  delta0 = deltai
c	else
          if (iparam .eq. 1) then
...
c	if ((jparam .le. 0) .or. (jparam .gt. 3)) then
        gammaj = gamma0
        deltaj = delta0
c	else
        if (jparam .eq. 1) then
...
c	rounding warning
     ...
     $     .or. (levy .and. (alpha .ne. 0.5d0) ) 
     $     .or. (anear1 .and. (alpha .ne. 1.0d0) )
     $     .or. (cauchy .and. (beta .ne. 0.0d0) ) ) ierr = warnrnd
